...
##' @export
##' @author Kai Guo
plot.venn<-function(x,type="venn",col="black",sep="_",mycol=c("dodgerblue",
                    "goldenrod1", "darkorange1", "seagreen3", "orchid3"),
                    ...
                    abbr=FALSE,abbr.method="both.sides",minlength=3,...)
{
    ...
    if(type=="vennpie"){
        print(vennpie(result,sep=sep,color=piecolor,revcolor=revcolor,any=any,
        show.number=show.number,show.x=show.x,log=log,base=base,
...
##' B <- sample(1:100, 60, replace = FALSE);
##' C <- sample(1:100, 40, replace = FALSE);
##' res1 <- venndetail(list(A = A,B = B), plot = FALSE)
##' res2 <- venndetail(list(A = A,C = C), plot = FALSE)
##' res <- merge(list(res1 = res1, res2 = res2), plot = TRUE)
##' @export
setMethod("merge",signature = (object="list"),function(object,ignore.case=FALSE,
