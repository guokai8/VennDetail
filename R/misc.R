...
setcolor<-function(n, palette = "default"){
    if (palette == "default") {
        ...
        categorical_colors <- c("#E41A1C", "#377EB8", "#4DAF4A",
        ...
        if (n <= length(categorical_colors)) {
            ...
            return(c(categorical_colors, sample(colors(), n -
                length(categorical_colors), replace = FALSE)))
        }
    } else if (palette == "sequential") {
        sequential_blues <- c("#F7FBFF", "#DEEBF7", "#C6DBEF",
        ...
        diverging_colors <- c("#67001F", "#B2182B", "#D6604D",
