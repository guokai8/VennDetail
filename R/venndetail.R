...
##' @param mycol Character vector giving the filled color for circles.
...
venndetail<-function(x,plot=TRUE,filename=NULL,type="venn",col="black",sep="_",
                    mycol=c("dodgerblue", "goldenrod1", "darkorange1",
                            "seagreen3", "orchid3"),cat.cex=1.5,alpha=0.5,
                    cex=2,cat.fontface="bold",margin=0.05,abbr=FALSE,
                    minlength=3,abbr.method="both.sides",
    ...
    if(length(x)==1){
        ...
        ggname=c("Shared",paste(names(x)[c(1,2,3,4,5,6)],sep="",collapse = sep),
    ...
    if(plot==TRUE){
        ...
        n=length(x)
        p<-venn.diagram(x,filename = filename,col = col,
                    fill = mycol[seq_len(n)],alpha = alpha,cex=cex,
                    cat.col = mycol[seq_len(n)],cat.cex = cat.cex,
                    cat.fontface = cat.fontface,margin = margin)
        grid.draw(p)
        ...
        if(type=="upset"){
            if(length(x)<=5){
                upset(fromList(x), nsets = length(x),point.size=5,
                    sets.bar.color=mycol[seq_along(x)],text.scale = text.scale)
            }else{
                upset(fromList(x), nsets = length(x),point.size=5,
                    sets.bar.color=setcolor[length(x)],text.scale = text.scale)
            }
        }
    }
    return(result)
