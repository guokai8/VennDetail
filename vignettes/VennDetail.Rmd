...
publication by _Hinder et al_ [1]. The three DEG sets were obtained in three 
...
-- _Get_ function provides a way to extract subsets from the main result along 
with any available annotations. The parameter _group_ asks the users to give the 
...
detail(ven) 
head(Get(ven, group = c("Shared", "SCN")), 10)
```    
...
the subsets name by using _detail_ function.
...
```{r getfeature}
head(getFeature(ven, group = "Shared", rlist=T2DM))
```
...
### 2.5 Support Shiny Now
A shiny web application [VennDetail](http://hurlab.med.und.edu/VennDetail/) for 
those who are not familiar with the R analysis platform 
